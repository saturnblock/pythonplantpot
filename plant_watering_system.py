...
class WateringControl:
    ...
    def process_config_requests(self):
        ...
        Diese Funktion ist threadsicher implementiert.
        """
        with self.config_lock:
            try:
                ...
                if pump_request and pump_request.get("timestamp", 0) > self.last_manual_request_ts:
                    ...
                    if amount > 0:
                        threading.Thread(target=self.pump.pump_timer, args=(amount,), daemon=True).start()
    ...
    def run_main_loop(self):
        """
        Die Hauptschleife, die alle Aktionen steuert.
        """
        global watering_status, config
        print("Haupt-Steuerungsschleife gestartet.")
        ...
        while not self._stop_event.is_set():
            ...
            if time.time() >= next_watering_time:
                ...
                if watering_status.get("remaining_watering_cycles", 0) > 0:
                    if self.prewatercheck.water_tank(watering_amount) and \
                        ...
                        threading.Thread(target=self.pump.pump_timer, args=(watering_amount,), daemon=True).start()
    ...
    def start(self):
        ...
        self._timer_thread = threading.Thread(target=self.run_main_loop, daemon=True)
